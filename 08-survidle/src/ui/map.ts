...
export function mapHtml(world: World, state: GameState, ui: UiState, cal: Calendar): string {
  ...
  // Home, before it has a fire or a roof: this region's camp, unless you stand on its glyph.
  const campGlyph = toGlyph(state.regions[cur]?.campCell ?? -1);
  if (campGlyph >= 0 && campGlyph !== playerGlyph && !markerAt.has(campGlyph)) markerAt.set(campGlyph, { glyph: "x", cls: "mk-camp" });
  ...
  parts.push(`<i class="shade"></i>${walkSvg(world, state, playerCell, x0, y0, z)}</div>`);
  parts.push(
    `<div class="legend"><span>~ water</span><span>= ice</span><span>A spruce</span><span>T pine</span><span>Y birch</span><span>. meadow</span><span>" bog</span><span>n rock</span><span>^ fell</span><span class="accent">@ you</span><span class="camp-key">x camp</span><span>H shelter</span><span>F fire</span><span class="pl-key">underlined: something lies there</span><span class="walk-key"><svg viewBox="0 0 24 6"><polyline class="walk-ahead" points="1,3 23,3"/></svg> your walk, solid ahead, dashed behind</span><span class="fog-key">dark: never been there</span></div>`,
  );
  return parts.join("");
}
