...
const GUIDES: Record<GoalId, Omit<GoalGuide, "id" | "phase">> = {
  ...
  findUsefulCover: { reason: "Natural cover can break the wind before a built shelter is ready.", path: "Explore > Shelter" },
  makeUsefulShelter: { reason: "Weatherproof shelter can come from improving cover, working quickly, or building for longer.", path: "Build > Shelter" },
  testShelter: { reason: "A shelter proves its worth by keeping weather off the body.", prompt: "Use weatherproof cover through the offered rain." },
  ...
  readWeather: { reason: "A sky reading can turn a vague change into time and facts to plan around.", path: "Explore > Weather" },
  prepareWeather: { reason: "A reachable camp, local shelter, or prepared refuge can all answer coming weather.", prompt: "Choose a viable plan before the storm arrives." },
  surviveForecast: { reason: "Coming through the storm connects the forecast to the plan that answered it.", prompt: "Live through the same storm you read and prepared for." },
  remoteRefuge: { reason: "Weatherproof shelter beyond home makes a longer range usable.", prompt: "Prepare protection in another region." },
  fieldFire: { reason: "A carried fire kit can make warmth away from camp.", path: "Camp > Fire" },
  fieldMeal: { reason: "Cooking where you travel turns carried gear into independence.", path: "Camp > Food" },
  remoteStorm: { reason: "A known refuge and a forecast make weather beyond home a choice instead of a retreat.", prompt: "Ride out an offered storm away from camp." },
...
export function goalProgress(state: GameState, world: World | undefined, _cal: Calendar, id: GoalId): GoalProgressView {
  ...
  const oneStep: Partial<Record<GoalId, GoalStepView>> = {
    ...
    findUsefulCover: { label: "Find protection", done: Boolean(state.goals.done.findUsefulCover) },
    makeUsefulShelter: { label: "Reach weatherproof protection", done: Boolean(state.goals.done.makeUsefulShelter) },
    testShelter: { label: "Weather the offered rain", done: Boolean(state.goals.done.testShelter) },
    readWeather: { label: "Learn a new forecast fact", done: Boolean(state.goals.done.readWeather) },
    prepareWeather: { label: "Have a viable plan at onset", done: Boolean(state.goals.done.prepareWeather) },
    surviveForecast: { label: "Survive the forecast storm", done: Boolean(state.goals.done.surviveForecast) },
    remoteRefuge: { label: "Prepare shelter in another region", done: Boolean(state.goals.done.remoteRefuge) },
    fieldFire: { label: "Light away from camp", done: Boolean(state.goals.done.fieldFire) },
    fieldMeal: { label: "Cook at a field fire", done: Boolean(state.goals.done.fieldMeal) },
    remoteStorm: { label: "Weather the storm beyond home", done: Boolean(state.goals.done.remoteStorm) },
  };
