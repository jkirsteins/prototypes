...
export function advance(state: GameState, world: World, dtMinutes: number, opts: { nobody?: boolean; wildlife?: WildlifeMode; live?: boolean } = {}): void {
  ...
  if (wildlife === "detailed") {
    ...
    while (left > CARRY_EPSILON && !state.dead) {
      ...
      if (toWorld - elapsed <= CARRY_EPSILON) {
        ...
        step(state, world, rng, MAX_STEP, nobody, wildlife, opts.live ?? false, 0);
      }
    }
  ...
  for (let tick = 0; tick < ticks && (nobody || !state.dead); tick++) step(state, world, rng, MAX_STEP, nobody, wildlife, opts.live ?? false);
  if (state.dead && !nobody) state.advanceCarry = 0;
  state.rng = rng.s;
}

function step(state: GameState, world: World, rng: Rng, dt: number, nobody: boolean, wildlife: WildlifeMode, live: boolean, wildlifeDt = dt): void {
  state.minute += dt;
  ...
  stepWildlife(state, world, cal, rng, wildlifeDt, wildlife, live);
