...
function versionNineFixture(goals: Record<string, unknown>): { version: number; state: GameState & Record<string, unknown> } {
...
describe("one-way opportunity save migration", () => {
  ...
  it.each([
    ...
    const loaded = read(versionNineFixture({ done: done(completed) }));
  ...
  it("does not replay introduced leaves or already presented completions", () => {
    const loaded = read(versionNineFixture({ done: { site: true, drink: true }, introduced: { firewood: true } }));
  ...
  it("batches queued completions, messages, and the new selected leaf with stable notice identity", () => {
    const loaded = read(versionNineFixture({
  ...
  it("keeps explicit step progress instead of replacing it with an aggregate", () => {
    const loaded = read(versionNineFixture({ introduced: { firewood: true }, progress: { firewood: 9 }, stepProgress: { firewood: { wood: 4.5 } } }));
  ...
  it.each([
    ...
    const raw = versionNineFixture({ done: done(EARLY) });
  ...
  it("keeps an introduced weather chapter eligible across a survivor clock reset", () => {
    const loaded = read(versionNineFixture({ done: done(EARLY), introduced: { readWeather: true } }));
  ...
  it("maps old seasons and selects the next unfinished old seasonal leaf", () => {
    ...
    const loaded = read(versionNineFixture({ done: done([...authored, "spring"]), lastSeason: "summer" }));
    ...
    const finished = read(versionNineFixture({ done: done([...authored, "spring", "summer", "autumn", "winter"]) }));
  ...
  it("does not backfill wildlife collections from old life records", () => {
    const loaded = read(versionNineFixtureWithKill("deer"));
    expect(loaded.opportunities.discoveredAt["hunt:deer"]).toBeUndefined();
  ...
  it.each(["testShelter", "readWeather", "remoteStorm", "fieldFire", "fieldMeal"])("preserves the legacy %s reservation and home", (goal) => {
    ...
    const loaded = read(versionNineFixture({ opportunity: weather, chapter3HomeRegion: 5 }));
  ...
  it("defaults missing historical weather metrics without discarding the reservation", () => {
    const loaded = read(versionNineFixture({ opportunity: {
