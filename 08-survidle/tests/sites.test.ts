...
import { allPanesHtml, paneHtml } from "./pane";
...
describe("no camp until one is made", () => {
  ...
  it("the Do panel offers the siting and refuses the camp work, with no confirm to click through", () => {
    const { state, world } = newGame(2);
    const cal = calendar(state.minute, state.startDoy);
    // The panel draws one pane at a time, so the row is asked for where it
    // lives rather than in whichever pane happens to open first.
    expect(paneHtml(state, world, cal, "makeCamp")).toContain("Make camp here");
    expect(allPanesHtml(state, world, cal)).toContain("no camp here yet");
    // The confirm is for moving a camp; there is none to move, so the row does not ask.
    expect(paneHtml(state, world, cal, "makeCamp", undefined, { confirmCamp: true })).not.toContain("Move camp here?");
  });

  it("every camp-addressed task refuses in the same words", () => {
    const { state, world } = newGame(2);
    const cal = calendar(state.minute, state.startDoy);
    for (const id of ["night", "haul", "hang", "lightIndoors", "mend"] as const) {
      const o = check(state, world, cal, id, id === "mend" ? "leanTo" : undefined);
