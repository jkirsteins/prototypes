...
describe("the epitaph", () => {
  ...
  it("is deterministic for the reference seeds; trap yields more with larger capacities", () => {
    ...
    // outlasts the larder and the run ends on hunger with wood still banked (60 kg for seed
    // 17, 44 kg for seed 19). Frozen lingon under the snow open a berries row through the
    ...
    expect(epitaph(runReference(17, 60).record)).toMatchInlineSnapshot(`"Aldona Niemi. Day 24. Starved at camp, with nothing in the pack and 60 kg of firewood at camp."`);
    expect(epitaph(runReference(19, 60).record)).toMatchInlineSnapshot(`"Astrid Dahl. Day 25. Starved at camp, with nothing in the pack and 44 kg of firewood at camp."`);
  });
