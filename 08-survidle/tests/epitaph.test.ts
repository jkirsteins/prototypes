...
describe("the epitaph", () => {
  ...
  it("is deterministic for the reference seeds; trap yields more with larger capacities", () => {
    ...
    // outlasts the larder and the run ends on hunger with wood still banked (63 kg for seed
    // 17, 38 kg for seed 19). Seed 17 falls a third of a kilometre out, on the way back. Frozen lingon under the snow open a berries row through the
    ...
    expect(epitaph(runReference(17, 60).record)).toMatchInlineSnapshot(`"Aldona Niemi. Day 24. Starved 0.3 km from camp, with nothing in the pack and 63 kg of firewood at camp."`);
