...
| Strike trajectories | The authored clip is the source of truth for the LOOK; `guard-positions`' `trajectoryCurve` is the same motion as engine data, ships hand-authored so the engine is not blocked on this spec, and is RE-EXTRACTED from the approved asset at validation. Validation bounds their divergence: a curve that drifts from its clip means the simulation and the picture disagree about where the blade is - which for a strike decides the pose a bind freezes. |
